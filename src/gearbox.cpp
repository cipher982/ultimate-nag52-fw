                ...
                if (this->shifter_pos != last_position)
                {
                    if (lock_state)
                    {
                        if (this->shifter_pos == ShifterPosition::P)
                        {
                            ...
                            if (this->shift_adapter != nullptr && this->shift_adapter->save() != ESP_OK) {
                                ESP_LOGW("PARK SAVE", "Shift adaptation persistence failed");
                            }
                            if (this->tcc->save() != ESP_OK) {
                                ESP_LOGW("PARK SAVE", "TCC map persistence failed");
                            }
                            // Keep all Park writes under the same task ownership.
                            ...
                            if (resumed != ESP_OK) {
                                ESP_LOGE("PARK SAVE", "TCC timer resume failed: %s", esp_err_to_name(resumed));
                            }
                            } else {
                                ESP_LOGW("PARK SAVE", "Persistence skipped: unsafe state or guard unavailable");
                            }
                        }
