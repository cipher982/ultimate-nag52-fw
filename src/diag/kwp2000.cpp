...
void Kwp2000_server::process_read_data_local_ident(uint8_t* args, uint16_t arg_len) {
    ...
    } else if (args[0] == RLI_TCC_TRANSIENT) {
        DATA_TCC_TRANSIENT r = get_tcc_transient_data(this->gearbox_ptr);
        make_diag_pos_msg(SID_READ_DATA_LOCAL_IDENT, RLI_TCC_TRANSIENT, (uint8_t*)&r, sizeof(DATA_TCC_TRANSIENT));
